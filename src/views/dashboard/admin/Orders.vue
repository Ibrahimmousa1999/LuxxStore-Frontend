<template>
  <div>
    ...
    <h1 class="section-title mb-8">{{ t('dashboard.admin.orders') }}</h1>
    ...
    <div class="lux-card" style="padding:0;overflow:visible">
      ...
      <div v-else style="overflow-x:auto;position:relative">
        <div v-if="filteredOrders.length > 0" style="min-width:800px">
          <table class="lux-table" style="position:relative">
            <thead>
              <tr>
        ...
        <div v-else class="text-center py-16">
          ...
          <div style="font-size:14px;color:var(--text-dim)">Orders will appear here once customers start purchasing</div>
        </div>
      </div>
    </div>
            ...
            <div class="lux-card" style="background:var(--surface2);padding:0;overflow:hidden">
              <div v-for="item in selectedOrder.items" :key="item.id" style="padding:16px;border-bottom:1px solid var(--border-white);display:flex;gap:16px;align-items:center">
                ...
                <div style="flex:1">
                  ...
                  <div style="font-size:13px;color:var(--text-muted)">{{ item.quantity }}x {{ Number(item.price).toLocaleString() }} {{ t('common.sar') }}</div>
                </div>
                <div style="font-size:18px;font-weight:900;color:var(--gold)">{{ Number(item.total || item.price * item.quantity).toLocaleString() }} {{ t('common.sar') }}</div>
              </div>
            </div>
          </div>
            ...
            <div class="lux-card" style="background:var(--surface2);padding:16px">
              <div style="display:flex;flex-direction:column;gap:8px">
                <div style="display:flex;justify-content:space-between;font-size:14px;color:var(--text-muted)">
                  <span>Subtotal</span>
                  <span>{{ Number(selectedOrder.subtotal || selectedOrder.total).toLocaleString() }} {{ t('common.sar') }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;font-size:14px;color:var(--text-muted)">
                  <span>Shipping</span>
                  <span>{{ Number(selectedOrder.shipping || 0).toLocaleString() }} {{ t('common.sar') }}</span>
                </div>
                ...
                <div style="display:flex;justify-content:space-between;font-size:18px;font-weight:900;color:var(--gold)">
                  <span>Total</span>
                  <span>{{ Number(selectedOrder.total).toLocaleString() }} {{ t('common.sar') }}</span>
                </div>
              </div>
            </div>
          </div>
